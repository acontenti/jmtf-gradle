...
/**
 * Performs a separable convolution using a one dimensional kernel given as a
 * float-array
 * 
	...
	public void manipulate(JMTFImage input) {
		...
		for (int x = 0; x < input.getWidth(); ++x) { // rows
			for (int y = 0; y < input.getHeight(); ++y) { // cols
				...
				tmp[x + input.getWidth() * y] = JMTFImage.getColor(
						Math.max((int) vals[0], 0), Math.max((int) vals[1], 0),
						Math.max((int) vals[2], 0));
			}
		}
		...
		for (int x = 0; x < input.getWidth(); ++x) {
			for (int y = 0; y < input.getHeight(); ++y) {
				...
				for (int i = 0; i < kernel_size; ++i) { // kernel
					...
					if ((y + i - kernel_size / 2) < input.getHeight()
							&& (y + i - kernel_size / 2) >= 0) {
						color = tmp[(x + (y + i - kernel_size / 2)
								* input.getWidth())];
				...
				input.setPixel(
						x,
						y,
						JMTFImage.getColor(Math.max((int) vals[0], 0),
								Math.max((int) vals[1], 0),
								Math.max((int) vals[2], 0)));
			}
		}
